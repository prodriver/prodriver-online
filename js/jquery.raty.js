...
;(function($) {
	...
	$.fn.raty = function(settings) {
		...
		for (var i = 1; i <= opt.number; i++) {
			...
			$this
			.append('<img id="' + id + '-' + i + '" src="' + opt.path + starFile + '" alt="' + i + '" title="' + hint + '" class="' + id + '" align="absmiddle" />')
			.append((i < opt.number) ? '&nbsp;' : '');
		}
		...
		if (!opt.readOnly) {
			...
			if (opt.cancel) {
				var stars	= $('img.' + id),
					cancel	= '<img src="' + opt.path + opt.cancelOff + '" alt="x" title="' + opt.cancelHint + '" class="button-cancel" align="absmiddle" style="margin:5px 0 0 0" />';
